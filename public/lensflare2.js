(function () {
    ...
    function init() {
        cvs = document.getElementById("lensflare-canvas");
        ...
        // follow mouse
        cvs.addEventListener("mousemove", function (a) {
            mousePos = getMousePos(cvs, a);
            fire.update(mousePos);
        });

        // follow sunset update
        document.addEventListener('sunset', function (e) {
            //console.log("Event! " + e.detail + " " + e.detail.sunX + " " + e.detail.sunY);
            fire.update({ x: e.detail.sunX, y: e.detail.sunY });
        });
    }
